...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  h1,h2,h3,.brand-title{font-family:'Sora',system-ui,sans-serif;}
  header.topbar{background:linear-gradient(120deg,var(--petrol-900) 0%, var(--petrol-800) 55%, var(--petrol-700) 100%);color:#fff;padding:28px clamp(16px,4vw,48px);box-shadow:var(--shadow-md);position:relative;overflow:hidden;}
  header.topbar::after{content:"";position:absolute;right:-80px;top:-80px;width:260px;height:260px;background:radial-gradient(circle, rgba(255,255,255,0.06), transparent 70%);pointer-events:none;}
  .topbar-inner{max-width:1400px;margin:0 auto;display:flex;align-items:center;justify-content:space-between;gap:24px;flex-wrap:wrap;position:relative;z-index:1;}
  .brand-block{display:flex;align-items:center;gap:20px;}
  .brand-mark{height:64px;width:64px;border:1px solid rgba(255,255,255,0.32);border-radius:14px;display:grid;place-items:center;font-family:'Sora',sans-serif;font-weight:800;letter-spacing:-0.08em;background:rgba(255,255,255,0.08);}
  .brand-divider{width:1px;height:48px;background:rgba(255,255,255,0.25);}
  .brand-text h1{margin:0;font-size:clamp(20px,2.4vw,28px);font-weight:700;letter-spacing:-0.01em;}
  .brand-text p{margin:4px 0 0;font-size:14px;color:rgba(255,255,255,0.72);font-weight:500;}
  .update-block{display:flex;flex-direction:column;align-items:flex-end;gap:6px;color:rgba(255,255,255,0.85);font-size:13px;}
  .update-block label{text-transform:uppercase;letter-spacing:.06em;font-size:11px;color:rgba(255,255,255,0.55);font-weight:600;}
  .update-pill{background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.25);color:#fff;padding:8px 12px;border-radius:var(--radius-sm);font-weight:700;}
  main{max-width:1400px;margin:0 auto;padding:28px clamp(16px,4vw,48px) 64px;}
  .alert{background:#fff6e5;border:1px solid #f1c36b;color:#7a5100;border-radius:var(--radius-md);padding:14px 16px;margin-bottom:20px;font-weight:600;}
  ...
  @media (max-width:640px){.summary-grid{grid-template-columns:repeat(2,1fr);}.topbar-inner{flex-direction:column;align-items:flex-start;}.update-block{align-items:flex-start;}}
  .summary-card{background:var(--card);border-radius:var(--radius-md);padding:18px 18px 16px;box-shadow:var(--shadow-sm);border:1px solid var(--border);border-top:4px solid var(--accent,var(--petrol-700));}
  .summary-card .label{font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:.05em;color:var(--ink-soft);display:flex;align-items:center;gap:6px;}
  .summary-card .value{font-family:'Sora',sans-serif;font-size:32px;font-weight:700;margin-top:8px;color:var(--petrol-900);line-height:1;}
  .summary-card .dot{width:9px;height:9px;border-radius:50%;flex-shrink:0;}
  .filters{background:var(--card);border:1px solid var(--border);border-radius:var(--radius-md);padding:16px 18px;display:flex;gap:14px;flex-wrap:wrap;align-items:center;margin-bottom:26px;box-shadow:var(--shadow-sm);}
  .filter-field{display:flex;flex-direction:column;gap:6px;min-width:180px;}
  .filter-field.search{flex:1;min-width:260px;}
  .filter-field label{font-size:11px;text-transform:uppercase;letter-spacing:.05em;font-weight:700;color:var(--ink-soft);}
  select,input{height:40px;border:1px solid var(--border);border-radius:var(--radius-sm);padding:0 12px;background:#fff;color:var(--ink);font:inherit;outline:none;}
  select:focus,input:focus{border-color:var(--teal-500);box-shadow:0 0 0 3px rgba(28,140,153,0.12);}
  .filter-reset{height:40px;align-self:end;border:none;background:var(--petrol-800);color:#fff;border-radius:var(--radius-sm);padding:0 16px;font-weight:700;cursor:pointer;}
  .results-count{margin:0 0 18px;color:var(--ink-soft);font-size:14px;}
  .cards-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:18px;margin-bottom:34px;}
  @media (max-width:1100px){.cards-grid{grid-template-columns:repeat(2,1fr);}}
  @media (max-width:720px){.cards-grid{grid-template-columns:1fr;}}
  .project-card{background:var(--card);border:1px solid var(--border);border-radius:var(--radius-lg);padding:20px;box-shadow:var(--shadow-sm);display:flex;flex-direction:column;gap:14px;}
  .dept{font-size:11px;text-transform:uppercase;letter-spacing:.08em;font-weight:800;color:var(--petrol-700);}
  .card-row{display:flex;justify-content:space-between;gap:12px;align-items:flex-start;}
  .pname{font-family:'Sora',sans-serif;font-weight:800;font-size:18px;line-height:1.25;color:var(--petrol-900);}
  .badge{display:inline-flex;align-items:center;gap:7px;border-radius:999px;padding:7px 10px;font-size:12px;font-weight:800;white-space:nowrap;}
  .badge .dot{width:8px;height:8px;border-radius:50%;}
  .badge.exec{background:var(--status-exec-bg);color:var(--status-exec-fg);}
  .badge.exec .dot{background:var(--status-exec-dot);}
  .badge.pend{background:var(--status-pend-bg);color:var(--status-pend-fg);}
  .badge.pend .dot{background:var(--status-pend-dot);}
  .badge.espera{background:var(--status-espera-bg);color:var(--status-espera-fg);}
  .badge.espera .dot{background:var(--status-espera-dot);}
  .badge.concl{background:var(--status-concl-bg);color:var(--status-concl-fg);}
  .badge.concl .dot{background:var(--status-concl-dot);}
  .badge.atraso{background:var(--status-atraso-bg);color:var(--status-atraso-fg);}
  .badge.atraso .dot{background:var(--status-atraso-dot);}
  .meta-line{display:flex;align-items:center;gap:8px;color:var(--ink-soft);font-size:13px;}
  .progress-wrap{display:flex;flex-direction:column;gap:8px;}
  .progress-label{display:flex;justify-content:space-between;font-size:12px;font-weight:700;color:var(--ink-soft);}
  .progress-track{height:9px;background:#ECF0F1;border-radius:999px;overflow:hidden;}
  .progress-fill{height:100%;border-radius:999px;transition:width .3s ease;}
  .note{font-size:13px;line-height:1.45;color:var(--ink-soft);background:#F8FAFA;border-radius:var(--radius-sm);padding:10px 12px;}
  .empty-state{background:var(--card);border:1px dashed var(--border);border-radius:var(--radius-md);padding:30px;text-align:center;color:var(--ink-soft);}
  .section-title{display:flex;align-items:center;justify-content:space-between;margin:8px 0 14px;}
  .section-title h2{margin:0;font-size:20px;color:var(--petrol-900);}
  .table-wrap{background:var(--card);border:1px solid var(--border);border-radius:var(--radius-md);box-shadow:var(--shadow-sm);overflow:hidden;}
  .table-wrap-scroll{overflow-x:auto;}
  table{width:100%;border-collapse:collapse;min-width:860px;}
  thead th{text-align:left;font-size:11px;text-transform:uppercase;letter-spacing:.06em;color:var(--ink-soft);background:#F8FAFA;padding:14px 16px;border-bottom:1px solid var(--border);}
  tbody td{padding:15px 16px;border-bottom:1px solid var(--border);font-size:14px;vertical-align:middle;}
  tbody tr:last-child td{border-bottom:none;}
  tbody tr:hover{background:#FAFCFC;}
  .table-progress{display:flex;align-items:center;gap:10px;}
  .table-progress .progress-track{width:90px;}
  .table-progress span{font-size:12px;font-weight:600;color:var(--ink-soft);min-width:34px;}
...
const STATUS_CONFIG = {
  "Em execucao": { badge: "exec", bar: "var(--status-exec-dot)" },
  "Pendente": { badge: "pend", bar: "var(--status-pend-dot)" },
  "Em espera": { badge: "espera", bar: "var(--status-espera-dot)" },
  "Concluido": { badge: "concl", bar: "var(--status-concl-dot)" },
  "Atrasado": { badge: "atraso", bar: "var(--status-atraso-dot)" }
};
const ICONS = {
...
function badgeClass(status){ return (STATUS_CONFIG[status] && STATUS_CONFIG[status].badge) || "espera"; }
function barColor(status){ return (STATUS_CONFIG[status] && STATUS_CONFIG[status].bar) || "var(--status-espera-dot)"; }
function escapeHtml(str){
  return String(str ?? '').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
}
function renderSummary(lista){
  const counts = {"Em execucao": 0, "Pendente": 0, "Em espera": 0, "Concluido": 0, "Atrasado": 0};
  lista.forEach(p => { if (counts[p.status] !== undefined) counts[p.status]++; });
  const cards = [
    { label: "Total de projetos", value: lista.length, accent: "var(--petrol-700)", dot: null },
    { label: "Em execucao", value: counts["Em execucao"], accent: "var(--status-exec-dot)", dot: "var(--status-exec-dot)" },
    { label: "Pendentes", value: counts["Pendente"], accent: "var(--status-pend-dot)", dot: "var(--status-pend-dot)" },
    { label: "Em espera", value: counts["Em espera"], accent: "var(--status-espera-dot)", dot: "var(--status-espera-dot)" },
    { label: "Concluidos", value: counts["Concluido"], accent: "var(--status-concl-dot)", dot: "var(--status-concl-dot)" }
  ];
